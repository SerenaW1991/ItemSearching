...
def predict(centroids, curPixel):
	curB, curG, curR = curPixel
	
	curMin = 255**2*3+1
	label = -1

	for x in range(len(centroids)):
		dis = ((centroids[x][0]-curB)**2 + (centroids[x][1]-curG)**2 + (centroids[x][2]-curR)**2)
		if dis < curMin:
			curMin = dis
			label = x
	return label
...
def func_fastClustering(testPic, centers, domainRatio, DELTA):
	...
	for row in range(0, testImageRow):
		for col in range(0, testImageCol):
			...
			predResult = predict(centers, curPixel)
